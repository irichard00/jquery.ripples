	...
	initShaders() {
		...
		this.renderProgram = createProgram([
			...
			'void main() {',
				...
				') * undulateAmplitude;',
				'vec4 baseColor = texture2D(samplerBackground, backgroundCoord + offset * perturbance + undulateOffset);',
				'float waveStrength = smoothstep(0.01, 0.2, abs(height));',
				'float crest = smoothstep(0.0, 0.12, height);',
				'float trough = smoothstep(0.0, 0.12, -height);',
				'vec3 highlightTint = vec3(0.28, 0.44, 0.78);',
				'vec3 shadowTint = vec3(0.36, 0.21, 0.08);',
				'vec3 color = baseColor.rgb;',
				'color += highlightTint * waveStrength * 0.85;',
				'color -= shadowTint * trough * 0.85;',
				'color += highlightTint * crest * 0.2;',
				'color += vec3(specular * 0.65);',
